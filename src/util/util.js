...
const Utils = {
   ...
   */
  getHeader(req, key) {
    if (!_.isUndefined(req.headers[key])) {
  ...
  flattenComponentsForRender(components) {
    const flattened = {};
    this.eachComponent(components, (component, path) => {
  ...
  renderComponentValue(data, key, components) {
    ...
    if (component.multiple) {
      ...
      compValue.value = _.map(value, (subValue) => this.renderComponentValue({
        [key]: subValue
      }, key, components).value).join(', ');
      return compValue;
    }
    ...
    switch (component.type) {
      ...
      case 'signature':
        ...
        compValue.value = (_.isString(value) && _.startsWith(value, 'data:')) ? 'YES' : 'NO';
        break;
      case 'container':
      ...
      case 'datagrid': {
        ...
        _.each(value, (subValue) => {
          ...
          _.each(columns, (column, key) => {
            ...
            if (_.isString(subCompValue.value)) {
              compValue.value += `<td style="padding:5px 10px;">${subCompValue.value}</td>`;
            }
          });
          compValue.value += '</tr>';
        });
  ...
  removeProtectedFields(form, action, submissions) {
    ...
    this.eachComponent(form.components, (component, path) => {
      ...
      if (component.protected) {
        ...
        modifyFields.push(((fieldPath) => (sub) => {
    ...
    if (modifyFields.length > 0) {
      _.each(submissions, (submission) => {
        _.each(modifyFields, (modifyField) => {
